...
class ArticlePostModelTests(TestCase):
    """docstring for ArticlePostModelTests"""
    ...
    def test_was_created_recently_with_hourrs_before_article(self):
        # 若文章创建时间为几小时前，返回False
    ...
    def test_was_created_recently_with_days_before_article(self):
        ...
        author.save()
        days_before_article = ArticlePost(
            author=author,
            title='test3',
            body='test3',
...
class ArticlePostViewTests(TestCase):
    """docstring for ArticlePostViewTests"""
    ...
    def test_increase_views_but_not_change_updated_field(self):
        ...
        viewd_article = ArticlePost.objects.get(id=article.id)
        self.assertIs(viewd_article.updated - viewd_article.created < timezone.timedelta(seconds=0.1), True)